...
local function createModel(opt)
   ...
   local model_geo = nn.Sequential()
   local model_int = nn.ParallelTable(2)
   ...
   elseif opt.dataset == 'ckp' then
      -- Model type specifies number of layers for CIFAR-100 model
      assert((depth - 2) % 6 == 0, 'depth should be one of 20, 32, 44, 56, 110, 1202')
      local n = (depth - 2) / 6
      iChannels = 16
      print(' | ResNet-' .. depth .. ' CKP')
      ...
      model:add(layer(basicblock, 32, n, 2))
      model:add(layer(basicblock, 64, n, 2))
      model:add(Avg(12, 12, 1, 1))
      model:add(nn.View(64):setNumInputDims(3))
      model:add(nn.Linear(64, 7))


      model_geo:add(nn.Linear(64, 7))

      model_int:add(model)
      model_int:add(model_geo)
   else
